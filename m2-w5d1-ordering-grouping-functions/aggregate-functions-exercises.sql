...
select max(surfacearea/population) as pop_dens
from country 
where population is not null and population > 0
...
select surfacearea/population as pop_density, lifeexpectancy, name
from country
where population is not null and population >0 and lifeexpectancy is not null
...
select gnpold-gnp as gnp_diff, abs(gnpold-gnp), name
from country
...
select count(code), continent, array_agg(name)
from country
group by continent
