...
//	$('#presidents').children('div').filter(":last").addClass('endOfCentury');
	$('#presidents').children('h2').nextUntil('h2').filter(":last").addClass('endOfCentury');
}
...
function currencyPresidents() {
	$('#presidents').children("h2:contains('1900')").nextUntil("h2:contains('2000')").filter('[class*=currency]').addClass('appearOnCurrency');
}