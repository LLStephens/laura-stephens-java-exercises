...
select (gnp/population) as gnp_per_pop from country where continent = 'Europe'

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
select (2017-indepyear) as years_since_ind from country where indepyear is not null
